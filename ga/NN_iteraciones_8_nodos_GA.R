...
GA.fit <- ga(type = "permutation", fitness = tpsFitness, distMatrix = distance, lower = 1, 
             upper =8 , popSize = 12, maxiter = 5000, run = 1000, pmutation = 0.1, 
             monitor = NULL)
...
for (b in seq(1, B)) {
  ...
  GA.rep <- ga(type = "permutation", fitness = tpsFitness, distMatrix = distance, 
               lower = 1, upper = 8, popSize = 10, maxiter = 5000, run = 1000, 
...
plot(x, y, type = "n", asp = 1, xlab = "", ylab = "", main = "Ruta obtenida con GA para 8 puntos de visita")
